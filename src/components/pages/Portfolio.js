        ...
        <ul className="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-3 xl:gap-x-8">
          {files.map((file) => (
            <li key={file.source} className="relative">
                ...
                <div className="flex justify-center items-center absolute inset-0 focus:outline-none links">
                  ...
                  <div className='group-hover:block inline-block mouseover hidden'>
                    <div className='font-bold'>
                      <a className='sm:text-sm lg:text-lg xl:text-2xl mr-2 inline-block' target='_' href={file.deployLink}>{file.title}</a>
              ...
              <p className="pointer-events-none block text-sm font-medium text-gray-500">{file.techStack}</p>
            </li>
          ))}
        </ul>
      </div>
